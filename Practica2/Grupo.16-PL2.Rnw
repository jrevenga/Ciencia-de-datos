...
lineas <- readLines("datos2.4.1.txt")

# Inicializar vectores para x e y
x <- numeric()
y <- numeric()

# Iterar sobre las líneas y extraer los datos
for (linea in lineas) {
  datos <- as.numeric(strsplit(linea, " ")[[1]])
  x <- c(x, datos[1])
  y <- c(y, datos[2])
}
...
cat("Intercepto (alpha):", resultado$alpha, "\n")
cat("Coeficiente (beta):", resultado$beta, "\n")

# Leer el archivo del segundo subconjunto de datos
lineas <- readLines("datos2.4.2.txt")
...
# Leer el archivo del tercer subconjunto de datos
lineas <- readLines("datos2.4.3.txt")
...
# Leer el archivo del cuarto subconjunto de datos
lineas <- readLines("datos2.4.4.txt")
